...
int main() {
   ...
   for (i = 0; i < zipCodeState.size(); ++i) {
		...
		if(zipCodeState[i].getuserKey() == zip){
		    abbrev = zipCodeState[i].getuserValue();
		}
	...
	for (i = 0; i < abbrevState.size(); ++i) {
		...
		if(abbrevState[i].getuserKey() == abbrev){
		    state = abbrevState[i].getuserValue();
		}
	}
	...
	for (i = 0; i < statePopulation.size(); ++i) {
		...
		if(statePopulation[i].getuserKey() == state){
		    population = statePopulation[i].getuserValue();
